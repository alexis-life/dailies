...
export default function ContextoHistoryList({ games, isSignedIn, onEdit, onChanged }) {
            ...
            <div className="history-item" key={game.id}>
              <button type="button" className="history-item-row" onClick={() => toggleExpand(game.id)}>
                <div className="history-item-row-top">
                  ...
                  {(game.hints_used ?? 0) > 0 && (
                    <span className="ax-badge badge-count">
                      {game.hints_used} {game.hints_used === 1 ? 'hint' : 'hints'}
                    </span>
                  )}
              ...
              {expanded && (
                <div className="history-item-detail">
                  {game.solution?.word && (
                    <p className="text-meta history-answer">answer: {game.solution.word}</p>
                  )}
