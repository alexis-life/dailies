...
import { parseExpressoGrid } from '../lib/daydashGridText'

export default function ExpressoLogForm({ onSaved, editingEntry, onCancelEdit }) {
  ...
  useEffect(() => {
    ...
    setExpression(editingEntry.solution?.expression ?? '')
  ...
  function handleParsePaste() {
    ...
    const parsedGrid = parseExpressoGrid(pasteText)
  ...
  async function handleSubmit(e) {
    ...
    const solution = {
      ...(expression.trim() ? { expression: expression.trim().toLowerCase() } : {}),
      ...(grid ? { guesses: grid } : {}),
    }
    ...
    <form className="ax-card log-game-form" onSubmit={handleSubmit}>
      ...
      <div className="form-row">
        ...
        <textarea
          className="ax-input"
          rows={3}
          placeholder={'paste the "I played expresso.ac…" share text here…'}
          value={pasteText}
      ...
      <div className="form-row">
        <label className="label-micro">secret expression (optional)</label>
        <input
          className="ax-input"
          type="text"
          placeholder={'e.g. "no way"'}
          value={expression}
